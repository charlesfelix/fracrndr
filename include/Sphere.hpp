...
namespace Fr {
    ...
    {
    public:
        Sphere(const V3f &center, Real radius);
        virtual ~Sphere(){};
        virtual void setMaterial(const std::shared_ptr<Material> & material);
        virtual bool hit(const Ray & ray, Real tmin, Real tmax, HitRecord & hit_record) const;
        virtual bool intersect(const Ray & ray, Real &t) const;
